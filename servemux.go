...
type Handler interface {
	ProcessTask(context.Context, *TaskInfo) error
}
...
func (mux *ServeMux) ProcessTask(ctx context.Context, task *TaskInfo) error {
...
func (mux *ServeMux) Handler(t *TaskInfo) (h Handler, pattern string) {
	mux.mu.RLock()
	defer mux.mu.RUnlock()

	h, pattern = mux.match(b2s(t.Type))
	if h == nil {
...
func (mux *ServeMux) HandleFunc(pattern string, handler func(context.Context, *TaskInfo) error) {
	if handler == nil {
		panic("asynq: nil handler")
	}
...
func NotFound(ctx context.Context, task *TaskInfo) error {
	return fmt.Errorf("handler not found for task %q", b2s(task.Type))
}
...
type HandlerFunc func(context.Context, *TaskInfo) error

// ProcessTask calls fn(ctx, task)
func (fn HandlerFunc) ProcessTask(ctx context.Context, task *TaskInfo) error {
	return fn(ctx, task)
}
